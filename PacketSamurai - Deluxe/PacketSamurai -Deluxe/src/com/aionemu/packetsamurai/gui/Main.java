...
public class Main implements IUserInterface {
	...
	public void init() {
		...
		_frame = new JFrame("Packet Samurai [Aion-Engine - 4.5x edition]");
	...
	public synchronized void log(String text, boolean display) 
	{
		...
		if (_writeLog) {
			try {
				File outFile = new File("logs/log-"+_timeId+".txt");
				if (!outFile.exists())
	...
	public void showAboutDialog() {
		JOptionPane.showMessageDialog(this.getMainFrame(), "Packet Samurai Aion-Engine\n\nGilles Duboscq\nUlysses R. Ribeiro\n\n\nModified by Magenik", "About Packet Samurai", JOptionPane.PLAIN_MESSAGE);
	}
				...
				try {
					...
					if (pane != null) {
						...
						AbstractLogReader reader = AbstractLogReader
								.getLogReaderForFile(pane.getFileName());
						if (reader == null)
							return;
						reader.reloadParse();
					...
					PacketSamurai
							.getUserInterface()
							.log("smth went wrong on reloading protocols...");
					e.printStackTrace();
				}
			}
		});
