...
public class CollectedNpcDataLoader {
	...
	public static void load() {
		try {
			...
			PacketSamurai.getUserInterface().log("Size of collected_npc_data: " + collection.getNpcStat().size());
	...
	public static void add(int npcId, int level, int hp, int titleId, int npcType) {
		...
		if (isNew)
			PacketSamurai.getUserInterface().log("New npc data: " + npcData.toString());
	}
	...
	public static void save() {
		...
		PacketSamurai.getUserInterface().log("Saved : " + templateList.size() + " npc data!");
	}
