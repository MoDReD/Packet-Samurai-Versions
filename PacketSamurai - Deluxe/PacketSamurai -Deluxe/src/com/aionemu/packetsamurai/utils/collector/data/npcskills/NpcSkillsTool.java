...
public class NpcSkillsTool {
	...
	public static void load() {
		...
		try {
			...
			collection = (NpcSkillTemplates) unmarshaller.unmarshal(new File(
					"data/npc_skills/npc_skills.xml"));

			PacketSamurai.getUserInterface().log(
					"Size of NpcSkills: " + collection.getNpcskills().size());
			for (NpcSkillList npc : collection.getNpcskills()) {
				skillsByNpcId.put(npc.getNpcid(), npc);
			}

		} catch (JAXBException e) {
			PacketSamurai.getUserInterface().log("ERROR load npc_skills: " + e);
		}
	...
	public static void save() {
		...
		try {
			...
			marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT,
					new Boolean(true));
			marshaller.marshal(collection, new FileOutputStream(
					"data/npc_skills/npc_skills.xml"));
		...
		PacketSamurai.getUserInterface().log(
				"Saved : " + templateList.size() + " npc skills!");
	}
}
