...
public class NpcSkillExporter 
{
	...
	public void parse()
	{
		...
		try
		{
			...
			for (DataPacket packet : packets)
			{
				...
				else if ("SM_CASTSPELL_END".equals(name))
				{
					...
					stat.useCount++;
					if (stat.startHp < hpAtUse)
						stat.startHp = hpAtUse;
					if (stat.endHp > hpAtUse)
						stat.endHp = hpAtUse;
				}
			}
			...
			for (NpcSkillUse entry : npcInfoList.values())
			{
				...
				for (SkillData skillData : entry.skillsUsed.values())
				{
					for (SkillUseStat stat : skillData.skillsByLevel.values())
					{
						sb.append("\t<npcskill starthp=\"");
						sb.append(Short.toString(stat.startHp));
						sb.append("\" endhp=\"");
						sb.append(Short.toString(stat.endHp));
		...
		PacketSamurai.getUserInterface().log("NPC Skills Has Been Written Successfull");
	}
