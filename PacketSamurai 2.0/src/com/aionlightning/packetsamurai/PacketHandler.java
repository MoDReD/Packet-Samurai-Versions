	...
	public void processReceivedPacket(Packet p) {
		try {
			...
			if (_captor instanceof FileCaptor) {
				...
				if (tcpSession == null) {
					System.err.println("PacketHandler : receivePacket (from FileCaptor) : New TCPSession (" + sessionId
							+ ") - Port: " + port);
					...
					if (proto != null) {
						...
						// XXX future packets of this sessionid, will once again try
				...
				if (!GameSessionTable.getInstance().sessionExists(sessionId)) {
					try {
						...
						PacketSamurai
								.getUserInterface()
								.log(
										"ERROR: Failed to create new Log Session, an I/O error occurred when creating the respective log file.");
					} catch (IllegalStateException e) {
						PacketSamurai.getUserInterface().log(e.getMessage());
					}
				} else {
					GameSessionTable.getInstance().getSession(sessionId).receivePacket(tcpPacket,
							!_captor.isClientAddress(tcpPacket.src_ip));
				}
			}
		} catch (Throwable t) {
			/*
			 * If this methods throws an exception when called by a native jpcap
			 * method (ie loopPacket) i do get a full JVM crash
			 * (EXCEPTION_ACCESS_VIOLATION) thus im enclosing it on a try as a way
			 * to increase the logger stability
			 */
			PacketSamurai.getUserInterface().log(
					"SEVERE: ERROR: Please report the exception trace, the logger will attempt to proceed through. ("
							+ t.toString() + ")");
			t.printStackTrace();
		}
	}
