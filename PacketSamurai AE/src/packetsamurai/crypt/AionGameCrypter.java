...
public class AionGameCrypter implements ProtocolCrypter
{
    ...
    private boolean searchKey(byte[] raw, packetDirection dir)
    {
        ...
        if (dir ==  packetDirection.serverPacket && "SM_KEY".equals(packet.getName()))
        {
            ...
            if(part == null)
            {
            	PacketSamurai.getUserInterface().log("Check your protocol there is no part called 'key' which is required in packet 0x66 SM_KEY of the GS protocol.");
                return false;
            }
            key = part.getIntValue();
            key = (key - 0x3FF2CCC7) ^ 0xCD92E4D3;
