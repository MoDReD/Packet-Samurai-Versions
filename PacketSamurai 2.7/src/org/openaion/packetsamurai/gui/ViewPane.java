...
import org.openaion.packetsamurai.protocol.PacketId.IdPart;
import org.openaion.packetsamurai.session.DataPacket;
import org.openaion.packetsamurai.session.GameSessionViewer;
...
public class ViewPane extends JPanel
{
    ...
    public void updateCurrentPacket(boolean forced)
    {
        ...
        if (gp.getPacketFormat() != null && gp.isValid())
        {
            ...
            //opcode, 2.7 = 2 bytes, use Data Packet IdData like checksum now for proper display.
            if (gp.getPacketId() != null)
            {
                for (IdPart part : gp.getPacketId().getParts())
                {
                    ...
                    {
                    	addStyledText(Util.zeropad(Integer.toHexString(gp.getIdData()[i2]&0xff),2).toUpperCase(),"op");
                        if(i2 < bytes -1)
                            addStyledText(" ", "op");
                    }
                    addStyledText(" ", "base");
