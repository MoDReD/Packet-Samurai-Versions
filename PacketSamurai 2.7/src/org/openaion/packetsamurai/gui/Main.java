package org.openaion.packetsamurai.gui;
...
import org.openaion.packetsamurai.Captor;
import org.openaion.packetsamurai.IUserInterface;
import org.openaion.packetsamurai.PacketSamurai;
import org.openaion.packetsamurai.gui.logrepo.LogRepoTab;
import org.openaion.packetsamurai.gui.protocoleditor.ProtocolEditor;
import org.openaion.packetsamurai.logreaders.AbstractLogReader;
import org.openaion.packetsamurai.session.DataPacket;
import org.openaion.packetsamurai.session.GameSessionTable;
import org.openaion.packetsamurai.session.Session;
import org.openaion.packetsamurai.utils.NpcGatherExporter;
import org.openaion.packetsamurai.utils.NpcInfoExporter;
import org.openaion.packetsamurai.utils.NpcShoutExporter;
import org.openaion.packetsamurai.utils.NpcSpawnExporter;
import org.openaion.packetsamurai.utils.NpcStateExporter;
import org.openaion.packetsamurai.utils.NpcStaticExporter;
import org.openaion.packetsamurai.utils.NpcTitleExporter;
import org.openaion.packetsamurai.utils.NpcWalkExporter;
import org.openaion.packetsamurai.utils.PetFeedExporter;

//import org.openaion.packetsamurai.utils.NpcPositionExporter;
...
public class Main implements IUserInterface {
	...
	public void init() {
		_frame = new JFrame("Packet Samurai [Elite Edition - 2.7+]");
		...
		exportInfos.setActionCommand("ExportNpcInfos");
		exportInfos.addActionListener(_menuListener);
		_utilsMenu.add(exportInfos);

		JMenuItem exportTitles = new JMenuItem("Export Npc Title IDs");
		exportTitles.setActionCommand("ExportNpcTitles");
		exportTitles.addActionListener(_menuListener);
		_utilsMenu.add(exportTitles);

		JMenuItem exportStatics = new JMenuItem("Export Npc Static IDs");
		exportStatics.setActionCommand("ExportNpcStatics");
		exportStatics.addActionListener(_menuListener);
		_utilsMenu.add(exportStatics);

		JMenuItem exportStates = new JMenuItem("Export Npc States IDs");
		exportStates.setActionCommand("ExportNpcStates");
		exportStates.addActionListener(_menuListener);
		_utilsMenu.add(exportStates);
/*
		JMenuItem exportPositions = new JMenuItem("Export Npc Position XYZ");
		exportPositions.setActionCommand("ExportNpcPositions");
		exportPositions.addActionListener(_menuListener);
		_utilsMenu.add(exportPositions);
*/
		JMenuItem exportShouts = new JMenuItem("Export Npc Shouts");
		exportShouts.setActionCommand("ExportNpcShouts");
		exportShouts.addActionListener(_menuListener);
		_utilsMenu.add(exportShouts);
		...
		JMenuItem exportNpcWalk = new JMenuItem("Export Npc Walk");
		exportNpcWalk.setActionCommand("ExportNpcWalk");
		exportNpcWalk.addActionListener(_menuListener);
		_utilsMenu.add(exportNpcWalk);
		
		JMenuItem exportPetFeed = new JMenuItem("Export Pet Feed");
		exportPetFeed.setActionCommand("ExportPetFeed");
		exportPetFeed.addActionListener(_menuListener);
		_utilsMenu.add(exportPetFeed);
	...
	private void exportNpcStates() {
		if (getViewerTab().getComponentCount() > 0) {
			...
			if (pane != null) {
				String sessionName = pane.getGameSessionViewer().getSession().getSessionName();
				FastList<DataPacket> packets = pane.getGameSessionViewer().getSession().getPackets();
				new NpcStateExporter(packets, sessionName).parse();
				
			}
		}
	}
/*
	private void exportNpcPositions() {
		if (getViewerTab().getComponentCount() > 0) {
			...
			if (pane != null) {
				String sessionName = pane.getGameSessionViewer().getSession().getSessionName();
				FastList<DataPacket> packets = pane.getGameSessionViewer().getSession().getPackets();
				new NpcPositionExporter(packets, sessionName).parse();
				
			}
		}
	}
*/
	private void exportNpcShouts() {
		if (getViewerTab().getComponentCount() > 0) {
			...
			if (pane != null) {
				String sessionName = pane.getGameSessionViewer().getSession().getSessionName();
				FastList<DataPacket> packets = pane.getGameSessionViewer().getSession().getPackets();
				new NpcShoutExporter(packets, sessionName).parse();
	...
	private void exportPetFeed() {
		if (getViewerTab().getComponentCount() > 0) {
			...
			if (pane != null) {
				String sessionName = pane.getGameSessionViewer().getSession().getSessionName();
				FastList<DataPacket> packets = pane.getGameSessionViewer().getSession().getPackets();
				new PetFeedExporter(packets, sessionName).parse();
			}
		}
	}
	...
	public class MenuActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent ev) {
			...
			} else if (actionCmd.equals("ExportNpcInfos")){
				exportNpcInfos();
			} else if (actionCmd.equals("ExportNpcTitles")){
				exportNpcTitles();
			} else if (actionCmd.equals("ExportNpcStatics")){
				exportNpcStatics();
			} else if (actionCmd.equals("ExportNpcStates")){
				exportNpcStates();
			}/* else if (actionCmd.equals("ExportNpcPositions")){
				exportNpcPositions();
			}*/ else if (actionCmd.equals("ExportNpcShouts")){
				exportNpcShouts();
			} else if (actionCmd.equals("ExportNpcSpawns")){
				exportNpcSpawns();
			} else if (actionCmd.equals("ExportGatherSpawns")){
				exportGatherSpawns();
			} else if (actionCmd.equals("ExportNpcWalk")){
				exportNpcWalk();
			} else if (actionCmd.equals("ExportPetFeed")) {
				exportPetFeed();
			}
